...
export function knowledgeText(r: AppRates): string {
 ...
 2) ${first > 0 ? `১০টি স্লট সম্পন্ন করলে ইউজার নিজে পায় ${tk(first)} বোনাস${r.promo ? " (এখন স্পেশাল অফার চলছে)" : ""}।` : `প্রথম ১০টি স্লট ভেরিফাই করলে এখন কোনো বোনাস নেই — বোনাসটা পাবেন ঐ ১০টি স্লট রি-ভেরিফাই করার পর। কখনো শুধু “০৳ বোনাস পাবেন” বলবে না, সুন্দর করে বুঝিয়ে বলবে।`}
 3) এরপর ৪ দিন পর প্রতিটি স্লট আবার রি-ভেরিফাই করতে হয়। ১০টি স্লট রি-ভেরিফাই সম্পন্ন হলে নতুন ইউজার ${tk(re)} বোনাস পায়। কিন্তু মাইনিং চালু হতে ১০টির দরকার নেই — <b>১টি স্লট রি-ভেরিফাই করলেই ওই ১টির মাইনিং চালু</b> (প্রতি স্লট ৫০৳/মাস, দিনে ≈১.৬৭৳)। যারা আগে রি-ভেরিফাই করেছিল, তারা নতুন সাইকেলে আবার করলে <b>প্রতি স্লটে ১০৳</b> সাথে সাথে মেইন ব্যালেন্সে পায়।
...
export function withdrawEligibilityReply(name: string): string {
  const openLine = `✅ তারিখের কোনো নিয়ম নেই — মেইন ব্যালেন্স ও আনলক হওয়া মাইনিং ব্যালেন্স <b>যেকোনো সময়</b> withdraw করা যায়।`;

  return (
    `${name}, মাইনিং ব্যালেন্সের <b>আনলক</b> অংশ এখনই withdraw করা যাবে। কোনো স্লটের টাকা লক থাকলে সেই স্লট রি-ভেরিফাই করলেই আনলক হবে 🔓\n\n` +
    `${openLine}\n\n` +
    `🎁 <b>বোনাস</b> থাকলে সেটা যেকোনো সময় withdraw করা যায়।\n` +
    `⛏️ <b>মাইনিং</b> টাকা যেকোনো সময় withdraw করা যায় — শুধু লক থাকা অংশের জন্য ওই স্লটটি রি-ভেরিফাই করতে হবে।\n\n` +
  ...
  switch (topic) {
    case "withdraw":
      return (
        ...
        `⏳ তারিখের কোনো নিয়ম নেই — মেইন ব্যালেন্স ও আনলক হওয়া মাইনিং ব্যালেন্স <b>যেকোনো সময়</b> তোলা যাবে। কোনো স্লটের মাইনিং টাকা লক থাকলে সেই স্লট রি-ভেরিফাই করলেই আনলক হবে 🔓`
      );
    case "password":
      return (
