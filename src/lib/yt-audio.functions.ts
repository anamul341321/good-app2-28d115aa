...
/**
 * YouTube ভিডিওর সরাসরি audio stream URL বের করে আনে।
 * এতে app minimize/screen-off হলে native MediaPlayer দিয়ে গান চালানো যায়
 * (iframe embed background-এ চলে না)।
 */
const PIPED_HOSTS = [
...
function pickPiped(json: any): string | null {
  ...
  const best = streams
    .filter((s) => s?.url && (s?.mimeType?.includes("audio") ?? true))
    .sort((a, b) => Number(b?.bitrate ?? 0) - Number(a?.bitrate ?? 0))
    .find((s) => Number(s?.bitrate ?? 0) <= 160_000) ?? streams[0];
...
function pickInvidious(json: any): string | null {
  ...
  const audio = formats
    .filter((f) => String(f?.type ?? "").startsWith("audio/") && f?.url)
    .sort((a, b) => Number(b?.bitrate ?? 0) - Number(a?.bitrate ?? 0));
  const best = audio.find((f) => Number(f?.bitrate ?? 0) <= 160_000) ?? audio[0];
  return best?.url ? String(best.url) : null;
}

export const getYoutubeAudioStream = createServerFn({ method: "POST" })
  .inputValidator((i: unknown) =>
    z.object({ videoId: z.string().min(5).max(24) }).parse(i),
  )
  .handler(async ({ data }) => {
