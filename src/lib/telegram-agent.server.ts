  ...
  try {
    ...
    if (name === "fee_quote") {
      ...
      if (!Number.isFinite(amount) || amount <= 0) {
        return "নিয়ম: উইথড্র ফি: ১০০৳ এর কম হলে ২০%, ১০০৳ বা তার বেশি হলে ১০%। সর্বনিম্ন ৬৩৳ (ফি ১২.৬০৳, হাতে ৫০৳, বাকি ০.৪০৳ মেইন ব্যালেন্সে থাকে)।";
      }
...
export async function agentAnswer(opts: {
  ...
  const system =
    ...
    `"ভাইয়া, আপনার ঐ স্লটটিতে এখনো কোনো ফেস ভেরিফিকেশন শুরু করা হয়নি, স্লটটি একদম খালি আছে — তাই ওখানে ডিলিট বা রিসেট করার মতো কিছুই নেই 🙂 ` +
    ...
    `  ৩) এককালীন বোনাস (শুধু একবার): রেট সবসময় নিচের database তথ্য থেকেই বলবে, নিজে কোনো অংক বানাবে না।\n` +
    `  ⚠️ ১ম ভেরিফাই বোনাস যদি ০৳ হয়, তখন কখনো শুধু "০৳ বোনাস পাবেন" বলবে না — সুন্দর করে বলবে: "প্রথম ১০টি ভেরিফাইয়ে এখন কোনো বোনাস নেই, কিন্তু ঐ ১০টি স্লট রি-ভেরিফাই করলেই রি-ভেরিফাই বোনাসটা পেয়ে যাবেন এবং মাইনিং চালু হবে।"\n` +
    `  🙏 অ্যাডমিন/মালিক (@anamulmunni) মেনশন করে কিছু জিজ্ঞেস করলে সবসময় সম্মান দিয়ে "জি স্যার" বলে উত্তর দেবে এবং যা জানতে চেয়েছেন তা গুছিয়ে দেবে; কখনো এলোমেলো বা অভদ্র উত্তর দেবে না।\n` +
