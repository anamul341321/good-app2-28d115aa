...
function SocialPeoplePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 pb-20">
      <header className="sticky top-0 z-40 bg-white border-b shadow-sm pt-[env(safe-area-inset-top)] px-4 py-3">
      ...
      <main className="flex-1 max-w-md mx-auto w-full p-4 space-y-4">
        {isLoading ? (
          ...
          <>
            {allUsers.map((u: any) => (
              <div key={u.id} className="bg-white p-3 rounded-xl border flex items-center justify-between">
                <button 
                ...
                >
                  ...
                  <div>
                    <p className="font-black text-navy">{u.display_name || "User"}</p>
                    <p className="text-xs text-gray-500 font-bold">UID {u.uid_seq}</p>
                  </div>
                </button>
