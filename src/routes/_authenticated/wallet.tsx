  ...
  return (
    <div className="space-y-4 pt-2">
      ...
      <div className="text-center">
        ...
        <p className="text-[11px] text-muted-foreground">বিকাশ ও নগদ — দুটোই আলাদা আলাদা সেট করা যাবে</p>
      </div>
      ...
      <ProviderCard
        ...
        offMessage={payout.bkashOffMessage}
        onSaved={refetch}
      />
      <ProviderCard
        provider="nagad"
        ...
        offMessage={payout.nagadOffMessage}
        onSaved={refetch}
      />

      <p className="text-[10px] text-muted-foreground text-center leading-snug px-3">
        একবার সেভ করার পর user নিজে নম্বর পরিবর্তন করতে পারবেন না — admin panel থেকে reset করাতে হবে।
      </p>
    </div>
  );
}
...
function ProviderCard({ provider, wallet, enabled, offMessage, onSaved }: {
      ...
      <div className={`glass rounded-2xl p-4 space-y-2 border-2 border-${tone}/30`}>
        ...
        {!enabled && (
          <div className="rounded-lg bg-rose/10 border border-rose/30 p-2 text-[11px] text-rose font-bold">
            ⚠️ {label} withdraw বর্তমানে বন্ধ। {offMessage || `${provider === "bkash" ? "নগদ" : "বিকাশ"}-এ withdraw দিন।`}
          </div>
        )}
      </div>
    );
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <p className={`text-sm font-black text-${tone} flex items-center gap-1.5`}>
          <span className="text-lg">{emoji}</span> {label} নম্বর যোগ করুন
        </p>
        {!enabled && (
          <span className="text-[9px] font-black uppercase tracking-widest text-rose">অস্থায়ীভাবে বন্ধ</span>
        )}
      </div>
      <input
        value={number}
      ...
      <p className="text-[10px] text-amber flex items-center gap-1">
        <ShieldCheck className="w-3 h-3" /> সঠিক {label} নম্বর দিন — সেভ করার পর নিজে পরিবর্তন করা যাবে না।
      </p>
      <button
        disabled={mut.isPending || number.length !== 11}
