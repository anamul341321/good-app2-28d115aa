ALTER TABLE public.mining_state ADD COLUMN IF NOT EXISTS self_qualified boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  SELECT coalesce(sum(greatest(1, floor(rv / 10.0)))::int, 0) INTO qual_ref
    FROM (
      SELECT (SELECT count(DISTINCT t2.slot) FROM public.tasks t2
                WHERE t2.user_id = p.id
                  AND coalesce(t2.reverify_count, 0) > 0) AS rv
       ...
       WHERE p.referred_by = _user_id
         ...
         AND (
           (SELECT count(DISTINCT t3.slot) FROM public.tasks t3
             WHERE t3.user_id = p.id
               AND coalesce(t3.reverify_count, 0) > 0) >= 10
           OR fv_mode
         )
    ) q;
  ...
  IF m.is_active AND m.last_credited_at IS NOT NULL THEN
    ...
    -- Self mining only accrues when the user themself was qualified
    -- (own 10 re-verified slots, or admin forced).
...
-- Backfill the flag from current data so existing users are graded correctly.
UPDATE public.mining_state ms
   SET self_qualified = (
     SELECT count(DISTINCT t.slot) FROM public.tasks t
