...
BEGIN
  ...
  -- Count every submitted slot that still has a valid whitelist. A slot can be
  -- waiting for re-verify (verified) or already re-verified (done); both count.
  SELECT count(*) INTO valid_count
  FROM public.tasks
  WHERE user_id = _user_id
    AND status IN ('verified','done')
