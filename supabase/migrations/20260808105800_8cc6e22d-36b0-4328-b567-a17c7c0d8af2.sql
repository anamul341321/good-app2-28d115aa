...
-- Mark existing eligible referees as already processed so the new logic
-- does not retro-pay every historical referral at once.
WITH fv AS (
  SELECT user_id, count(DISTINCT slot) c
  FROM public.tasks WHERE slot <= 10 AND initial_verify_at IS NOT NULL
  GROUP BY 1
)
UPDATE public.profiles p
   SET referrer_bonus_paid_at = now()
  FROM fv
 WHERE fv.user_id = p.id AND fv.c >= 10
   AND p.bonus_first_verify_claimed = true
   AND p.referrer_bonus_paid_at IS NULL;
...
BEGIN
  ...
  -- Referrer bonus: tracked by its own timestamp so it can never be skipped
  -- because of the referee's own bonus flags.
