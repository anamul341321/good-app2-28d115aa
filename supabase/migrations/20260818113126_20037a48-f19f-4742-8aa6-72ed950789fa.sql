...
BEGIN
  ...
  -- Slot must be whitelisted (re-verify complete) — otherwise both the 10 BDT
  -- gift and that slot's mining stay locked until the user re-verifies.
  IF NOT v_ok THEN
  ...
  -- Sweep whatever this slot has mined since the reward was parked, so saving up
  -- for several days never loses money.
  ...
  VALUES (_user_id, v_bonus, 'slot_claim', _task_id,
          jsonb_build_object('bonus', v_bonus, 'mining_moved_to_main', v_mining, 'total', v_total, 'slot', v_slot));
